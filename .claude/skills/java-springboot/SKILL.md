...
- **Build Tool:** Always use Maven (`pom.xml`) dependency management.
...
- **Package Structure:** Always organize code by layer (e.g., `com.example.app.controller`, `com.example.app.service`).
...
- **Externalized Configuration:** Use `application.yml`. YAML is preferred for its readability and hierarchical structure.
...
- **Projections:** Use DTO projections to fetch only the necessary data from the database. Don't load whole entity when you need few columns. Tell JPA to select only those columns, map straight into small object (projection). Less data over wire, less
  memory, faster.
